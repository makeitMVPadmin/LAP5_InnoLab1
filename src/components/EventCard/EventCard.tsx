...
const EventCard: React.FC<EventCardProps> = ({
  ...
  const tagStyle =
    "flex items-center justify-center px-4 font-gilroy h-7 rounded-full";
    ...
    <article className="flex flex-col w-full md:w-80 h-[34.5625rem] mb-4">
      <section className="relative ">
        ...
        >
          <img
            ...
            src={isFavorite || isHovered ? HeartIconActive : HeartIconInactive}
            alt="favorite icon"
          />
        </button>
        <Link to={`/join-event/${id}`}>
          <div
            className="relative h-[18.4375rem] border-[3px] border-MVP-black rounded-[24.2px] bg-cover bg-center w-full"
            style={{ backgroundImage: `url(${imageUrl})` }}
          >
            <div className="absolute flex gap-2 flex-wrap top-3 right-2.5">
              {joined && (
                <span
                  className={`${tagStyle} border border-MVP-black bg-MVP-yellow text-black`}
                >
                  Joined
                </span>
              )}
            ...
            <div className="absolute flex items-end justify-between flex-wrap bottom-[1.0625rem] w-[92%] left-2">
              <span
                className={`${tagStyle} border-MVP-black bg-MVP-white text-MVP-black border-2`}
              >
                {skillLevel}
              </span>
            </div>
      ...
      <section className="flex flex-col justify-between pt-4 gap-4 flex-1 font-poppins">
        <div className="flex flex-col gap-4">
          <h3 className="text-[1.5625rem] font-bold overflow-hidden text-ellipsis">
            {title}
          </h3>
          ...
          <div className="flex flex-wrap gap-2 text-[1rem] leading-none">
            {themesWithIds.map((theme) => (
              <span
                key={theme.id}
                className="bg-MVP-white text-MVP-black gap-1 font-poppins h-[1.6169rem] leading-4"
              >
                {theme.name}
              </span>
            ))}
